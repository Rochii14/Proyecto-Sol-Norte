...
---------------------- importar archivo 2024 que tiene distinto formato
CREATE OR ALTER PROCEDURE ddbbaTP.Importar_Clima2
...
BEGIN
    ...
    WHERE NOT EXISTS (
        SELECT 1
        FROM ddbbaTP.Dia_LLuvia AS f
        WHERE f.Fecha = d.Fecha
    );
   
    SELECT COUNT(*) AS TotalDiasInsertados FROM ddbbaTP.Dia_LLuvia; -- Confirmar días
END;
GO
EXEC ddbbaTP.Importar_Clima2 'C:\open-meteo-buenosaires_2024.csv' --Guardamos el archivo en extensión .csv UTF-8
go
-----------
select * from ddbbaTP.Dia_LLuvia
